...
namespace Laba
{
    public partial class ExcelBigText : Component
    {
        public ExcelBigText()
        {
            InitializeComponent();
        }
        public ExcelBigText(IContainer container)
        {
            container.Add(this);
        ...
        public void SaveExcel(string path, string title, string[] strings)
        {
            if (string.IsNullOrEmpty(path) || string.IsNullOrEmpty(title) || strings.Length == 0)
            {
                throw new ArgumentException("Empty Strings");
            }
            CreateExcel(path);

            InsertCellInWorksheet("A", 1, title, 2);
            uint index = 2;
            foreach(string i in strings)
            {
                InsertCellInWorksheet("A", index, i, 0);
                index++;
            }
        ...
        private void InsertCellInWorksheet(string ColumnName, uint RowIndex, string Text, uint Style)
        {
